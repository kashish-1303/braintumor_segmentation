...
# print("Loading data...")
# X_patches = np.load("x_training.npy", mmap_mode='r')
# Y_labels = np.load("y_training.npy", mmap_mode='r')

# # Use only 1% of the data
# num_samples = int(X_patches.shape[0] * 0.01)
# X_patches = X_patches[:num_samples].astype(np.float32)
# Y_labels = Y_labels[:num_samples].astype(np.float32)
...
#         UpSampling2D(2),
#         Conv2D(4, 3, activation='softmax', padding='same')
#     ])
#     return model
...
# # Save the model
# model.save('simple_brain_seg_model.keras')
# print("Model saved.")